...
def main():
    # Fixed filenames
    if len(sys.argv) < 2:
        print("Usage: python updateJSON.py <base_name>")
        print("Reads <file_name>_bom_updated.graphml")
        print("Writes <file_name>_bom_updated.json")
        sys.exit(1)

    base_name = sys.argv[1]
    graphml_path = f"{base_name}_bom_updated.graphml"
    json_path = f"{base_name}_bom.json"
    output_path = f"{base_name}_bom_updated.json"
