(function() {
    ...
    // The function that adds the filtered results in a grid
    ...
    function showDetailModal(imageUrl, details) {
        ...
        //initiali values for creating a objectgeometry
        let objectThickness = '';
        let objectHeight = '';
        let objectWidth = ''; //add an explanation of how they are defined 
        ...
        console.log("There are the details of the object: ", details)
        ...
        details.forEach(function(detail) {
            ...
            if (detail.header === 'Width') {
                objectWidth = detail.value;
            }
            if (detail.header === 'Height') {
                objectHeight = detail.value;
            }
            if (detail.header === 'Thickness') {
                objectThickness = detail.value;
            }

            if (detail.header === 'NumberCode') {
            ...
            if (detail.header === 'Placement') {
                ...
                console.log("the element is external:" + objectIsExternal);
            }

            if (detail.header === 'LoadBearing') {
                objectIsLoadBearing = detail.value; 
                console.log(" the element is load bearing:" + objectIsLoadBearing);
            }

            if (detail.header === 'StiffeningFunction') {
                objectIsStiffening = detail.value; 
                console.log(" the element's fire Rating is :" + objectIsStiffening);
            }

            if (detail.header === 'FIRE REGULATION') {
                objectIsFireRated = detail.value; 
        ...
        document.getElementById('exportButton').addEventListener('click', function() {
            ...
            var fabrication = objectFabricationMethod ;
        ...
        ///RENDER THE GEOMETRY
        renderObject(objectCategory,objectWidth, objectHeight, objectThickness);
    }
    ...
    function createBoundingBox(objectCategory,objectWidth, objectHeight, objectThickness){
        ...
        var width = objectWidth*unitCnversionFactor;
        var height = objectHeight*unitCnversionFactor;
        var depth = objectThickness*unitCnversionFactor;
    ...
    //to do update functio to take jsut one argumewnt the geometry
     // render the object using the three.js 
    function renderObject(objectCategory,objectWidth, objectHeight, objectThickness) {
        ...
        // create a mesh 
        var elementGeometry = new THREE.Mesh(elementBBox, material);
        ...
        var animate = function() {
            ...
            elementGeometry.rotation.y += 0.006;
